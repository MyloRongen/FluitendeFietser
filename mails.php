...
$sql = "SELECT ID, naam, achternaam, email, vraag from verzenden";  
$resultaat = $con-> query($sql);

if ($resultaat-> num_rows > 0) {
  while ($row = $resultaat-> fetch_assoc()) {
    echo "<tr><td>" . $row["naam"] . "</td><td>" . $row["achternaam"] . "</td><td>" . $row["email"] . "</td><td>" . $row["vraag"] . "</td></tr>";
  }
  echo "</table>";
}
else {
