...
$sql = "SELECT id, naam, email, adres, stad, postcode, prijs from fiethuren";  
$resultaat = $con-> query($sql);

if ($resultaat-> num_rows > 0) {
  while ($row = $resultaat-> fetch_assoc()) {
    echo "<tr><td>" . $row["naam"] . "</td><td>" . $row["email"] . "</td><td>" . $row["adres"] . "</td><td>" . $row["stad"] . "</td><td>" . $row["postcode"] . "</td><td>" . $row["prijs"] . "</td></tr>";
  }
  echo "</table>";
}
else {
